...
65C0'],
    ...
    const performanceChartData = {
        ...
        datasets: [
            {
                label: 'Count',
                data: [agentData.metrics.assigned, agentData.metrics.active, agentData.metrics.sold, agentData.totalReviews],
                backgroundColor: ['rgba(45, 90, 39, 0.2)', 'rgba(21, 101, 192, 0.2)', 'rgba(255, 152, 0, 0.2)', 'rgba(32, 201, 151, 0.2)'],
                borderColor: ['#2D5A27', '#1565C0', '#ff9800', '#20c997'],
                borderWidth: 2,
                borderRadius: 8,
    ...
    if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-zinc-500">Loading Agent Launchpad...</div>;
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="relative overflow-hidden bg-zinc-900 border border-zinc-800 p-8 rounded-3xl shadow-2xl">
                <div className="relative z-10 flex justify-between items-start flex-wrap gap-6">
                    <div>
                        <h1 className="text-4xl font-bold text-white mb-2">Welcome back, {agentData.name} 👋</h1>
                        <p className="text-zinc-400 text-lg">Manage your assigned properties, site visits, and negotiations.</p>
                    </div>
                    <div className="flex gap-3">
                        <button className="px-5 py-2 bg-white text-black font-bold rounded-xl hover:bg-zinc-200 transition-all flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                {[
                    { label: 'Assigned Properties', value: agentData.metrics.assigned, icon: '🏢', color: 'text-blue-500' },
                    { label: 'Active Listings', value: agentData.metrics.active, icon: '✅', color: 'text-green-500' },
                    { label: 'Under Negotiation', value: agentData.metrics.negotiating, icon: '🤝', color: 'text-yellow-500' },
                    { label: 'Sold', value: agentData.metrics.sold, icon: '💰', color: 'text-emerald-500' },
                    { label: 'Rating', value: agentData.rating, icon: '⭐', color: 'text-yellow-400' },
                    { label: 'Reviews', value: agentData.totalReviews, icon: '💬', color: 'text-zinc-400' },
                ].map((stat, i) => (
                    <div key={i} className="bg-zinc-900 border border-zinc-800 p-5 rounded-3xl flex flex-col items-center text-center hover:border-zinc-600 transition-all">
                        <div className="text-2xl mb-2">{stat.icon}</div>
                        <div className={`text-2xl font-bold ${stat.color}`}>{stat.value}</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden">
                    ...
                    <div className="divide-y divide-zinc-800">
                        ...
                        {upcomingVisits.length === 0 && <div className="p-10 text-center text-zinc-500">No upcoming visits</div>}
                    </div>
                </div>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden">
                    ...
                    <div className="divide-y divide-zinc-800">
                        ...
                        {recentOffers.length === 0 && <div className="p-10 text-center text-zinc-500">No offers yet</div>}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    ...
                    {assignedProperties.length === 0 && <div className="p-10 text-center text-zinc-500">No properties assigned yet.</div>}
                </div>
            </div>
        </div>
    );
